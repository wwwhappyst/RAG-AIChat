...
```powershell
& 'D:\dev_tools\anaconda3\python.exe' ask.py "coze平台怎么注册并开始实践？" --top-k 3 --show-debug
```
...
```powershell
& 'D:\dev_tools\anaconda3\python.exe' ask.py
```
...
```powershell
& 'D:\dev_tools\anaconda3\python.exe' -m py_compile config.py src\prompt_builder.py src\generator.py src\pipeline.py ask.py
```
